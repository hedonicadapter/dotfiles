...
module.exports = (() => {
  const config = {
    info: {
      ...
      github:
        "https://github.com/arg0NNY/DiscordPlugins/tree/master/BetterAnimations",
      github_raw:
    ...
    changelog: [
      {
        type: "fixed",
        title: "Fixed",
        items: ["Fixed an error preventing settings from opening."],
      },
      {
        type: "progress",
        title: "Stay tuned",
        items: ["Big changes are underway..."],
      },
    ],
  };
    ...
    : (([Plugin, Api]) => {
        const plugin = (Plugin, Api) => {
          ...
          function getMangled(filter) {
            const target = WebpackModules.getModule(
              (m) => Object.values(m).some(filter),
              { searchGetters: false },
            );
            return target
              ? [target, Object.keys(target).find((k) => filter(target[k]))]
              : [];
          }
          ...
          const ChannelIntegrationsSettingsWindow = WebpackModules.getByProps(
            "setSection",
            "saveWebhook",
          );
          // const {PreloadedUserSettingsActionCreators} = WebpackModules.getByProps('PreloadedUserSettingsActionCreators');
          const { Route: RouteWithImpression } = WebpackModules.getByProps(
            "Route",
            "Router",
          );
          const UserPopout = WebpackModules.getModule(
            (m) => m?.type?.toString?.().includes("Unexpected missing user"),
            { searchExports: true },
          );
            ...
            Object.entries(selectors).forEach(([key, selector]) => {
              ...
              Object.defineProperty(result, key, {
                get: () => {
                  if (memoized === null) {
                    memoized = getter();
                    if (!memoized || !Object.keys(memoized).length)
                      setTimeout(() => (memoized = null), 5000);
                  }
          ...
          const Selectors = buildSelectors({
            ...
            PeopleTab: () =>
              WebpackModules.getByProps("container", "peopleColumn"),
            ApplicationStore: () =>
              WebpackModules.getByProps("applicationStore", "marketingHeader"),
            PeopleList: () =>
              WebpackModules.getByProps("peopleList", "searchBar"),
            FriendsActivity: () =>
              WebpackModules.getByProps("scroller", "header", "container"),
            PageContainer: () =>
              WebpackModules.getByProps("headerBar", "homeWrapper"),
            ...
            Members: () =>
              WebpackModules.getByProps("members", "hiddenMembers"),
            EmojiPicker: () =>
              WebpackModules.getByProps(
            ...
            StickerPicker: () =>
              WebpackModules.getAllByProps(
                "listWrapper",
                "loadingIndicator",
              )?.filter((m) => !m?.gridNoticeWrapper)[0],
            GifPicker: () =>
              WebpackModules.getByProps("searchBar", "backButton"),
            Popout: () =>
              WebpackModules.getByProps("disabledPointerEvents", "layer"),
            ThreadSidebar: () =>
              WebpackModules.getByProps("container", "resizeHandle"),
            Stickers: [
            ...
            Colors: () =>
              WebpackModules.getByProps("colorHeaderPrimary", "colorWhite"),
            VideoOptions: [
              () => WebpackModules.getByProps("backgroundOptionRing"),
              { backgroundOptionRing: "backgroundOptionRing__45698" },
            ],
            StudentHubs: () =>
              WebpackModules.getByProps("footerDescription", "scroller"),
          });
          ...
          const GuildIdHistory = new History(
            SelectedGuildStore.getGuildId() ?? "@me",
          );
          ...
          class Route {
            constructor(
              name,
              path,
              { element, scrollers, getter, forceGuildChange, noGuilds },
            ) {
              this.name = name;
            ...
            get _guildSwitched() {
              return (
                !this.noGuilds &&
                (this.forceGuildChange ||
                  GuildIdHistory.previous !== GuildIdHistory.current)
              );
            }

            get element() {
              return !this._guildSwitched
                ? this._getter
                  ? this._getter().element
                  : this._element
                : `.${Selectors.Layout.base}`;
            }

            get scrollers() {
              return [
                ...(this._getter ? this._getter().scrollers : this._scrollers),
                ...(this._guildSwitched
                  ? [Selectors.ChannelsList.scroller]
                  : []),
              ];
            }
          }
          const Routes = [
            new Route(
              ...
              ],
              {
                ...
                scrollers: [
                  Selectors.Messages.scroller,
                  DiscordClasses.MemberList.members.value,
                  Selectors.Content.scrollerBase,
                ],
              },
            ),
            new Route("Friends", "/channels/@me", {
              ...
              scrollers: [
                Selectors.PeopleList.peopleList,
                Selectors.FriendsActivity.scroller,
              ],
            }),
          ...
          class ContainerAnimator {
            static TYPES = {
              ...
              slipUp: (node, clonedNode, { duration, easing, offset }) =>
                ContainerAnimator.TYPES.slipVertical(node, clonedNode, {
                  duration,
                  easing,
                  offset,
                  upwards: true,
                }),
              slipDown: (node, clonedNode, { duration, easing, offset }) =>
                ContainerAnimator.TYPES.slipVertical(node, clonedNode, {
                  duration,
                  easing,
                  offset,
              ...
              slipVertical: (
                node,
                clonedNode,
                { duration, easing, offset, upwards },
              ) => {
                return new Promise((res) => {
                  node.animate(
                    {
                      transform: [
                        `translateY(${!upwards ? "-" : ""}${offset ?? 50}px)`,
                        `translateY(0)`,
                      ],
                      opacity: [0, 1],
                    },
              ...
              slipLeft: (node, clonedNode, { duration, easing, offset }) =>
                ContainerAnimator.TYPES.slipHorizontal(node, clonedNode, {
                  duration,
                  easing,
                  offset,
                  upwards: true,
                }),
              slipRight: (node, clonedNode, { duration, easing, offset }) =>
                ContainerAnimator.TYPES.slipHorizontal(node, clonedNode, {
                  duration,
                  easing,
                  offset,
              ...
              slipHorizontal: (
                ...
                return new Promise((res) => {
                  node.animate(
                    {
                      transform: [
                        `translateX(${!upwards ? "-" : ""}${offset ?? 50}px)`,
                        "translateX(0)",
                      ],
                      opacity: [0, 1],
                    },
              ...
              scaleForwards: (
                node,
                clonedNode,
                { duration, easing, scale, zIndex },
              ) =>
                ContainerAnimator.TYPES.scale(node, clonedNode, {
                  duration,
                  easing,
                  scale,
                  zIndex,
              ...
              scale: (
                node,
                clonedNode,
                { duration, easing, scale, forwards, zIndex },
              ) => {
                return new Promise((res) => {
                  scale = scale ?? 0.05;
              ...
              scaleUp: (
                ...
                ContainerAnimator.TYPES.scaleVertical(node, clonedNode, {
                  duration,
                  easing,
                  scale,
                  zIndex,
              ...
              scaleVertical: (
                node,
                clonedNode,
                { duration, easing, scale, upwards, zIndex },
              ) => {
                return new Promise((res) => {
                  scale = scale ?? 0.05;
                  ...
                  node
                    .animate(
                      [
                        {
                          transform: `translateY(${upwards ? "" : "-"}100%)`,
                          opacity: 0,
                        },
                        { transform: `translateY(0)`, opacity: 1 },
                      ],
                ...
                return new Promise((res) => {
                  ...
                  node
                    .animate(
                      [
                        {
                          transform: `translateX(${upwards ? "" : "-"}100%)`,
                          opacity: 0,
                        },
                        { transform: `translateX(0)`, opacity: 1 },
                      ],
              ...
              slideUp: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.slideVertical(node, clonedNode, {
                  duration,
                  easing,
                  upwards: true,
                }),
              slideDown: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.slideVertical(node, clonedNode, {
                  duration,
                  easing,
                  upwards: false,
              ...
              slideVertical: (
                node,
                clonedNode,
                { duration, easing, upwards },
              ) => {
                return new Promise((res) => {
                  node.animate(
                    {
                      transform: [
                        `translateY(${!upwards ? "-" : ""}100%)`,
                        "translateY(0)",
                      ],
                      opacity: [0, 1],
                    },
              ...
              slideLeft: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.slideHorizontal(node, clonedNode, {
                  duration,
                  easing,
                  upwards: true,
                }),
              slideRight: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.slideHorizontal(node, clonedNode, {
                  duration,
                  easing,
                  upwards: false,
              ...
              flipRight: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.flipVertical(node, clonedNode, {
                  duration,
                  easing,
                  forwards: true,
                }),
              flipLeft: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.flipVertical(node, clonedNode, {
                  duration,
                  easing,
                  forwards: false,
              ...
              flipVertical: (
                ...
                return new Promise((res) => {
                  ...
                  node.parentNode.style.perspectiveOrigin = `${
                    rect.left + rect.width / 2
                  }px ${rect.top + rect.height / 2}px`;
                  node.animate(
                    {
                      transform: [
                        `rotateY(${forwards ? "-" : ""}90deg)`,
                        `rotateY(${forwards ? "-" : ""}90deg)`,
                        `rotateY(0)`,
                      ],
                    },
              ...
              flipForwards: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.flipHorizontal(node, clonedNode, {
                  duration,
                  easing,
                  forwards: true,
                }),
              flipBackwards: (node, clonedNode, { duration, easing }) =>
                ContainerAnimator.TYPES.flipHorizontal(node, clonedNode, {
                  duration,
                  easing,
                  forwards: false,
            ...
            constructor(
              ...
              element,
              scrollSelectors = [],
              { elementToAppear, zIndex } = {},
            ) {
              if (!ContainerAnimator.TYPES.hasOwnProperty(type))
                return Logger.err("Invalid animation type.");
            ...
            init() {
              if (!this.node)
                return Logger.warn("Unable to find node to animate.");
              const rect = {
              ...
              if (getComputedStyle(this.parentNode).position === "static")
                this.parentNode.classList.add(PARENT_NODE_CLASSNAME);
              ...
              this.clonedNode.style.position =
                getComputedStyle(this.node).position === "fixed"
                  ? "fixed"
                  : "absolute";
              ...
              ["top", "left", "width", "height"].forEach(
                (p) => (this.clonedNode.style[p] = rect[p] + "px"),
              );

              const query = this.scrollSelectors.map((s) => "." + s).join(", ");
              const scrollers = Array.from(this.node.querySelectorAll(query));
              if (scrollers.length) {
                const clonedScrollers = Array.from(
                  this.clonedNode.querySelectorAll(query),
                );
                clonedScrollers.forEach(
                  (e, i) => (e.scrollTop = scrollers[i].scrollTop),
                );
              }
            ...
            animate(params = {}) {
              ...
              const exec = () => {
                ...
                this.animation(
                  this.elementToAppear ?? this.node,
                  this.clonedNode,
                  params,
                ).then(() => {
                  this.end();
          ...
          class RevealAnimator {
            static TYPES = {
              ...
              scaleForwardsSide: (
                node,
                { duration, easing, reverse, scale, position },
              ) => {
                return new Promise((res) => {
                  scale = scale ?? 0.1;
              ...
              rotateForwardsLeft: (
                node,
                { duration, easing, reverse, scale },
              ) =>
                RevealAnimator.TYPES.rotate(node, {
                  duration,
                  easing,
                  reverse,
                  scale,
              ...
              rotate: (
                node,
                { duration, easing, reverse, scale, left, forwards },
              ) => {
                return new Promise((res) => {
                  scale = scale ?? 0.1;
                  node
                      ...
                      [
                        {
                          opacity: 0,
                          transform: `rotate${
                            ["top", "bottom"].includes(position) ? "X" : "Y"
                          }(${
                            ["right", "bottom"].includes(position) ? "" : "-"
                          }90deg)`,
                        },
              ...
              slipScale: (
                node,
                { duration, easing, reverse, offset, position, scale },
              ) => {
                return new Promise((res) => {
                  scale = scale ?? 0.1;
            ...
            constructor(type, element, { needsCopy, scrollSelectors } = {}) {
              if (!RevealAnimator.TYPES.hasOwnProperty(type))
                return Logger.err("Invalid animation type.");
            ...
            copyNode() {
              ...
              this.node.parentNode.style.position =
                getComputedStyle(this.node.parentNode).position === "static"
                  ? "relative"
                  : "";
              ...
              ["top", "left", "width", "height"].forEach(
                (p) => (clonedNode.style[p] = rect[p] + "px"),
              );
              this.scrollSelectors.forEach((s) =>
                this.node.querySelector(`.${s}`)
                  ? Array.from(clonedNode.querySelectorAll(`.${s}`)).forEach(
                      (e, i) =>
                        (e.scrollTop = this.node.querySelectorAll(`.${s}`)[
                          i
                        ].scrollTop),
                    )
                  : null,
          ...
          return class BetterAnimations extends Plugin {
            ...
            patchPages() {
              ...
              Patcher.before(
                RouteWithImpression.prototype,
                "render",
                (self, _) => {
                  if (
                    self.props.path === undefined ||
                    (typeof self.props.path === "object" &&
                      self.props.path.length > 5)
                  )
                    return;

                  RoutePathHistory.push(
                    typeof self.props.path === "object"
                      ? self.props.path[0]
                      : self.props.path,
                  );
                  ...
                  if (
                    RouteLocationHistory.current ===
                    RouteLocationHistory.previous
                  )
                    return;

                  const current = Routes.find((r) =>
                    r.path.includes(RoutePathHistory.current),
                  );
                  const previous = Routes.find((r) =>
                    r.path.includes(
                      RoutePathHistory.previous ?? "/channels/@me",
                    ),
                  );
                  if (!current || !previous) return;

                  guildSwitched =
                    current._guildSwitched || previous._guildSwitched;

                  if (guildSwitched && !this.settings.guild.enabled) return;
                  if (!guildSwitched && !this.settings.channel.enabled) return;

                  this.setMainAnimator(
                    new ContainerAnimator(
                      guildSwitched
                        ? this.settings.guild.type
                        : this.settings.channel.type,
                      guildSwitched
                        ? `.${Selectors.Layout.base}`
                        : previous.element,
                      previous.scrollers,
                      {
                        elementToAppear: guildSwitched
                          ? `.${Selectors.Layout.base}`
                          : current.element,
                      },
                    ),
              ...
              Patcher.after(
                ...
                (self, _, value) => {
                  ...
                  const { duration, easing } = guildSwitched
                    ? this.settings.guild
            ...
            patchGuildDiscovery() {
              ...
              const before = (id) => {
                ...
                animator = new ContainerAnimator(
                  this.settings.channel.type,
                  `.${Selectors.Pages.pageWrapper}, .${Selectors.StudentHubs.scroller}`,
                  [Selectors.Content.scrollerBase],
                  {
                    elementToAppear: `.${
                      id === -2
                        ? Selectors.StudentHubs.scroller
                        : Selectors.Pages.pageWrapper
                    }`,
                  },
                );
              };
              ...
              const selectCategory = getMangled(
                (m) =>
                  m?.toString &&
                  m.toString().includes("GUILD_DISCOVERY_SELECT_CATEGORY"),
              );
              ...
              Patcher.before(...selectCategory, (self, _) => {
                ...
                if (
                  GuildDiscoveryCategoryHistory.current ===
                  GuildDiscoveryCategoryHistory.previous
                )
                  return;
            ...
            patchSettingsView() {
              ...
              const before = (animatorOverride) => {
                ...
                animator =
                  ...
                  new ContainerAnimator(
                    ...
                    `.${Selectors.Sidebar.contentRegion}`,
                    [
                      Selectors.Sidebar.contentRegionScroller,
                      Selectors.Settings.scroller,
                      Selectors.Content.scrollerBase,
                    ],
                    { zIndex: 110 },
                  );
              ...
              [
                ...
                Patcher.before(w, "setSection", (self, _) => {
                  ...
                  if (
                    SettingsSectionHistory.current ===
                    SettingsSectionHistory.previous
                  )
                    return;
              ...
              Patcher.before(GuildSettingsWindow, "selectRole", (self, _) => {
                ...
                if (
                  GuildSettingsRoleIdHistory.current ===
                  GuildSettingsRoleIdHistory.previous
                )
                  return;
                before();
              });
              ...
              Patcher.before(
                ChannelIntegrationsSettingsWindow,
                "setSection",
                (self, _) => {
                  ChannelIntegrationsSectionHistory.push(_[0]);

                  if (
                    ChannelIntegrationsSectionHistory.current ===
                    ChannelIntegrationsSectionHistory.previous
                  )
                    return;
                  before();
                },
            ...
            patchMessages() {
              ...
              this.messageMutationObserver = new MutationObserver((records) => {
                records.forEach((r) =>
                  r.addedNodes.forEach((n) => {
                    const node = n.id?.startsWith("chat-message")
                      ? n
                      : n.querySelector
                      ? n.querySelector('*[id^="chat-message"]')
                      : null;
                    if (!node) return;
                    ...
                    messageNode
                      .animate(
                        [
                          { height: 0, opacity: 0 },
                          {
                            height: messageNode.clientHeight + "px",
                            opacity: 0,
                          },
                        ],
                        {
              ...
              this.messageMutationObserver.observe(document, {
                childList: true,
                subtree: true,
              });
              ...
              this.messageCreateHandler = (e) => {
                ...
                if (
                  ...
                  e.message.state !== MessageStates.SENDING
                )
                  return;
            ...
            patchPopouts() {
              ...
              Patcher.before(
                ...
                (self, _) => {
                  ...
                  const node =
                    self.ref?.current ??
                    self.elementRef?.current ??
                    document.getElementById(self.props.id) ??
                    document.querySelector(`.${self.props.className}`);
                  if (!node) return;
              ...
              Patcher.before(UserPopout, "type", (self, props) => {
                if (
                  !popoutNode ||
                  popoutNode.children[0]?.className.includes("loadingPopout")
                )
                  return;
            ...
            patchExpressionPicker() {
              ...
              const before = (view) => {
                ...
                if (
                  ExpressionPickerViewHistory.current ===
                  ExpressionPickerViewHistory.previous
                )
                  return;

                const current = ExpressionPickerRoutes.find((r) =>
                  r.path.includes(ExpressionPickerViewHistory.current),
                );
                const previous = ExpressionPickerRoutes.find((r) =>
                  r.path.includes(ExpressionPickerViewHistory.previous),
                );
                if (!current || !previous) return;
                ...
                pickerAnimator = new ContainerAnimator(
                  this.settings.expressionPicker.type,
                  previous.element,
                  previous.scrollers,
                  { elementToAppear: current.element },
                );
              };
              ...
              const setExpressionPickerView = getMangled((m) =>
                m
                  ?.toString?.()
                  .match(
                    /\w+\.setState\({activeView:\w+,lastActiveView:\w+\.getState\(\)\.activeView}\)/,
                  ),
              );
              const toggleExpressionPicker = getMangled((m) =>
                m
                  ?.toString?.()
                  .match(
                    /\w+\.getState\(\)\.activeView===\w+\?\w+\(\):\w+\(\w+,\w+\)/,
                  ),
              );

              Patcher.before(...setExpressionPickerView, (self, _) =>
                before(_[0]),
              );
              Patcher.before(...toggleExpressionPicker, (self, _) =>
                before(_[0]),
              );
            ...
            injectCss() {
              ...
              DOM.addStyle(
                ...
                `
                            ...
                            /* Settings View Fix */
                            /* .${Selectors.Sidebar.standardSidebarView}, .${
                              Selectors.Sidebar.contentRegionScroller
                            } {
                                background: var(--background-tertiary) !important;
                            }

                            .${Selectors.Settings.contentContainer}, .${
                              Selectors.SettingsSidebar.sidebar
                            } {
                                background: var(--background-primary) !important;
                            } */

                            .${CLONED_NODE_CLASSNAME}.${
                              Selectors.Sidebar.contentRegion
                            } *, .${CLONED_NODE_CLASSNAME}.${
                              Selectors.Settings.contentContainer
                            } * {
                            ...
                            .${
                              Selectors.EmojiPicker.emojiPickerHasTabWrapper
                            }, .${Selectors.StickerPicker.wrapper}, .${
                              Selectors.GifPicker.container
                            } {
            ...
            getSettingsPanel() {
              ...
              class RevealAnimationExample extends React.Component {
                ...
                componentDidMount() {
                  const animate = () => {
                    ...
                    const node =
                      this.ref.current.querySelector('[data-page="1"]');
                    ...
                    this.props.animation(node, params).then(() => {
                      setTimeout(() => {
                        ...
                        this.props
                          .animation(
                            node,
                            Object.assign(params, { reverse: true }),
                          )
                          .then(() => {
              ...
              class ContainerAnimationExample extends React.Component {
                ...
                componentDidMount() {
                  const animate = () => {
                    ...
                    const pages = [
                      node.querySelector('[data-page="1"]'),
                      node.querySelector('[data-page="2"]'),
                    ];
                    ...
                    this.props
                      .animation(pages[this.state.page], pages[page], {
                        duration: this.props.getOption().duration,
                        easing: Easing[this.props.getOption().easing],
                        offset: 10,
                        scale: 0.3,
                    ...
                    return React.createElement(
                      ...
                      [
                        React.createElement(
                          ...
                          [
                            React.createElement(component, {
                              animation:
                                this.props.animations[this.props.type.key],
                              getOption,
                            }),
                            React.createElement(
                              "div",
                        ...
                        React.createElement("div", {
                          className:
                            Selectors.VideoOptions.backgroundOptionRing,
                          style: {
                            display: this.props.type.selected
                              ? "block"
                              : "none",
                            borderRadius: "10px",
                          },
              ...
              const Buttons = (...props) => {
                ...
                class Panel extends React.Component {
                  render() {
                    ...
                    return React.createElement("div", {}, buttons);
                  }
                }
              ...
              const element = Settings.SettingPanel.build(
                ...
                new Settings.SettingGroup("Guild Animations").append(
                  new Settings.Switch(
                    "Enable guild switching animation",
                    null,
                    this.settings.guild.enabled,
                    (e) => {
                  ...
                  new Settings.Dropdown(
                    "Easing",
                    "Easing functions can be viewed at www.easings.net",
                    this.settings.guild.easing,
                    easings,
                    (e) => {
                ...
                new Settings.SettingGroup("Channel Animations").append(
                  new Settings.Switch(
                    "Enable channel switching animation",
                    null,
                    this.settings.channel.enabled,
                    (e) => {
                  ...
                  new Settings.Slider(
                    ...
                    100,
                    5000,
                    this.settings.channel.duration,
                    (e) => {
                ...
                new Settings.SettingGroup("Settings Animations").append(
                  new Settings.Switch(
                    "Enable settings sections switching animation",
                    null,
                  ...
                  new Settings.Slider(
                    ...
                    100,
                    5000,
                    this.settings.settings.duration,
                    (e) => {
                ...
                new Settings.SettingGroup("Messages Animations").append(
                  new Settings.Switch(
                    "Enable new message reveal animation",
                    null,
                  ...
                  new Settings.Dropdown(
                    "Side",
                    "Sets the side for side-dependent animations.",
                    this.settings.messages.position,
                    positions,
                    (e) => {
                ...
                new Settings.SettingGroup(
                  ...
                  new Settings.Switch(
                    "Enable popouts, context menus and tooltips animations",
                    null,
                  ...
                  new Settings.Slider(
                    ...
                    100,
                    5000,
                    this.settings.popouts.duration,
                    (e) => {
                ...
                new Settings.SettingGroup(
                  ...
                  new Settings.Switch(
                    "Enable expression picker sections switching animation",
                    null,
                    this.settings.expressionPicker.enabled,
                    (e) => {
                  ...
                  new Settings.SettingField(
                    ...
                    AnimationTypes(
                      ...
                      ContainerAnimator.TYPES,
                      ContainerAnimationExample,
                      () => this.settings.expressionPicker,
                    ),
                  ),
                  ...
                  new Settings.Dropdown(
                    "Easing",
                    "Easing functions can be viewed at www.easings.net",
                    this.settings.expressionPicker.easing,
                    easings,
                    (e) => {
                  ...
                  new Settings.Slider(
                    ...
                    100,
                    5000,
                    this.settings.expressionPicker.duration,
                    (e) => {
            ...
            closeSettings() {
              document
                .querySelector(".bd-addon-modal-footer > .bd-button")
                .click();
            }
            ...
            checkForUpdates() {
              PluginUpdater.hasUpdate(config.info.github_raw).then(
                (hasUpdate) => {
                  if (hasUpdate)
                    return Toasts.info("Plugin update has been found");
