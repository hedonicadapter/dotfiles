...
with lib; let
  ...
  set_circadian_vars = pkgs.writeTextFile {
    name = "set_circadian_vars";
    executable = true;
    destination = "/bin/anti-sleep-neglector";
    text = ''
      #!/usr/bin/env bash
  ...
  circadian-monitor-brightness = pkgs.writeTextFile {
    name = "circadian-monitor-brightness";
    executable = true;
    destination = "/bin/anti-sleep-neglector";
    text = ''
      #!/usr/bin/env bash
  ...
  circadian-wallpapers = pkgs.writeTextFile {
    name = "circadian-wallpapers";
    executable = true;
    destination = "/bin/anti-sleep-neglector";
    text = ''
      swww-daemon &
...
in {
  ...
  config = mkIf cfg.enable {
    programs.zsh.initExtra = ''
      ...
      alias get_circadian_periods='export FIRST_LIGHT=$(get_circadian_period FIRST_LIGHT) DAWN=$(get_circadian_period DAWN) SUNRISE=$(get_circadian_period SUNRISE) SOLAR_NOON=$(get_circadian_period SOLAR_NOON) SUNSET=$(get_circadian_period SUNSET) LAST_LIGHT=$(get_circadian_period LAST_LIGHT) || echo "Failed to update sun variables"'
    '';
    ...
    systemd.user.services."anti-sleep-neglector" = {
      ...
      Service = {
        Type = "oneshot";
        ExecStart = "${set_circadian_vars}/bin/anti-sleep-neglector";
      };
    ...
    systemd.user.services."anti-sleep-neglector-monitor" = {
      ...
      Service = {
        Type = "oneshot";
        ExecStart = "${circadian-monitor-brightness}/bin/anti-sleep-neglector";
      };
    ...
    systemd.user.services."anti-sleep-neglector-wallpaper" = {
      ...
      Service = {
        Type = "oneshot";
        ExecStart = "${circadian-wallpapers}/bin/anti-sleep-neglector";
      };
    ...
    systemd.user.timers."anti-sleep-neglector-wallpaper.timer" = {
      Unit = {
      ...
      Timer = {
        OnBootSec = "5s";
        OnUnitActiveSec = "45m";
        Unit = "anti-sleep-neglector-wallpaper.service";
      };
