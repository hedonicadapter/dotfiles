...
---@return string
local i_CR = function()
	return compleet.is_completion_selected() and "<Plug>(compleet-insert-selected-completion)" or "<Plug>delimitMateCR"
end
...
local close = function()
	...
	if filetype == "startify" then
		vim.cmd("bd")
	elseif filetype == "help" or buftype == "nofile" or #vim.fn.getbufinfo({ buflisted = 1 }) == 1 then
		vim.cmd("q")
...
local setup = function()
	...
	keymap.set("i", "<C-e>", 'pumvisible() ? "\\<C-e>" : "\\<C-o>g$"', { expr = true, noremap = true })

	-- Move between displayed lines instead of physical ones.
	keymap.set({ "n", "v" }, "<Up>", "g<Up>", { noremap = true })
	keymap.set("i", "<Up>", 'pumvisible() ? "<C-p>" : "<C-o>g<Up>"', { expr = true, noremap = true })
	keymap.set({ "n", "v" }, "<Down>", "g<Down>", { noremap = true })
	keymap.set("i", "<Down>", 'pumvisible() ? "<C-n>" : "<C-o>g<Down>"', { expr = true, noremap = true })
	...
	if has_cokeline then
		...
		for i = 1, 9 do
			keymap.set("n", ("<F%s>"):format(i), ("<Plug>(cokeline-focus-%s)"):format(i))
		end
	end
